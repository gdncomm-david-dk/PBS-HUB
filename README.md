# PBS Hub — Ops Console PCF (Dashboard, Report, Report Detail)

Power Apps code components (PCF) untuk canvas app PBS Hub, dibuat dari design handoff
*PBS Ops Console* (artboard 3a/3b, 4a, 4b–4d) dengan data mapping mengikuti `DESIGN.md`
(list SharePoint v1: `Report - PBS Hub`, `Report Automation - PBS Hub`, `Schedule - PBS Hub`,
`Clock In - PBS Hub`, `Host`, `Studio`, `Brand`, `Payroll`).
...
**Output:** `dist/PBSHubOpsPCF_1_0_0_0_managed.zip` — managed solution, dibangun dengan target MSBuild
...
```
shared/            logika + UI bersama (dipakai ketiga control)
...
npm run build            # build 3 control (pcf-scripts, production)
npm run typecheck
...
(`?c=Dashboard`, `?c=ReportReview`, `?c=ReportDetail&r=RPT-20862`). `node harness/flows.js <dir>` menjalankan
cek interaksi (approve → mengirim → hasil, konflik, revisi, bulk approve, filter, paging).
