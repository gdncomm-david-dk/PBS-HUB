...
#   PBSHubHostPCF — Host app controls (pbs_Host.*), a separate solution so each app ships on its own
#   PBSHubHostSchedulePCF — Host schedule controls (pbs_Host.MySchedule, pbs_Host.ScheduleDetail), shipped on their own
# Needs: .NET SDK (8+) — the cdsproj pulls Microsoft.PowerApps.MSBuild.Solution/Pcf from NuGet.
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p dist
for SOL in ${SOLUTIONS:-PBSHubOpsPCF PBSHubHostPCF PBSHubHostSchedulePCF}; do
