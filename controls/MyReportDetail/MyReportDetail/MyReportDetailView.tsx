...
import { Row, date, nameIndex, person, rowId, str } from "../../../shared/data";
import { fmtDateTimeShort, fmtLongDate, fmtSignedPct, fmtTime } from "../../../shared/format";
import {
  HOST_REPORT_STATE,
  HostSession,
  MetricValues,
...
function SubmitReport(props: MyReportDetailProps & { session: HostSession | undefined }): React.ReactElement {
  ...
  if (sent) {
    return (
      <div className="hc-card" style={{ textAlign: "center", padding: "32px 20px" }}>
        ...
        <Button onClick={() => action.fire("BACK", {})}>Kembali ke Report saya</Button>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      ) : res?.action === "ABSEN" && res.status === "ok" ? (
...
function Revision(props: Omit<MyReportDetailProps, "report"> & { report: Row; session: HostSession | undefined }): React.ReactElement {
